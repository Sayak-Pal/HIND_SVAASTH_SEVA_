  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (data.text) {
        ...
        setMessages([...newMessages, { role: "model", content: "Error: No response from Gemini." }])
      }
    } catch (error) {
      console.error("Error fetching Gemini response:", error)
      setMessages([...newMessages, { role: "model", content: "Error: Could not fetch response." }])
    }
          ...
          <CardContent className="flex flex-col flex-1 p-4 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto space-y-4">
              {messages.map((msg, idx) => (
                ...
                >
                  {msg.role === "model" && <Bot className="h-5 w-5 mt-1" />}
                  <div
